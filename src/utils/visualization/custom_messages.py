...
def show_rewards(mode, day, roi, sharpe, reward, metric_mean):
    message = f"\033[1;36m {mode.upper()} >> TD[{day}]: ROI: {np.round(roi, 2)}, SHARPE RATIO: {np.round(sharpe, 2)}, and REWARD: {np.round(reward, 2)}. AVG REWARD: {np.round(metric_mean, 2)}.\033[0m"
    print(message)
