...
def portfolio_management(day, prev_action, curr_action, position, cash, token_portfolio, current_token_prices, current_gas_price, priority_fee, gas_limit, tokens_to_buyorhold, tokens_to_sell):
    ...
    if position == "Hold":
        ...
        total_cash_remaining, total_tokens_value_remaining, curr_total_net_worth = hold_position(day=day, curr_action=curr_action, cash=cash, tokens_to_hold=tokens_to_buyorhold, portfolio=token_portfolio, token_prices=current_token_prices)
    if position == "Buy":
    ...
    if position == "Sell":
        ...
        total_cash_remaining, total_tokens_value_remaining, curr_total_net_worth, token_portfolio = sell_position(day=day, curr_action=curr_action, tokens=tokens_to_sell, base_gas=current_gas_price, gas_limit=gas_limit, priority_fee=priority_fee, portfolio=token_portfolio, token_prices=current_token_prices)
    if position == "Swap":
